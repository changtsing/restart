...
func GetInheritListeners(timeout time.Duration) (*net.TCPListener, net.Conn, error) {
	...
	if err != nil {
		if ope, ok := err.(*net.OpError); ok && ope.Op == "accpect" {
			log.Printf("getInheritListeners accpet unix timeout, %s", timeout)
			return nil, nil, err
		}
	...
	if len(scms) != 1 {
		log.Printf("GetInheritListeners ParseSocketControlMessage failed: expected 1 bug get scms = %#v\n", scms)
		return nil, nil, err
	}
...
func ListenNextInherit(waittime time.Duration, wg *sync.WaitGroup, stopChan chan bool, transChan chan bool) {
	...
	if err != nil {
		log.Printf("[warning]:	ListenNextInherit faield %s\n", err)
		return
	}
